...
tiles = 49
datasize = 1000
biosample = "bc_organoid_normalization_over_all_channels_"
...
def main_DataGenerator(filepath,per,patchsize,channels,l_layer,u_layer,tiles,biosample,datasize):
    ...
    single_stack = np.vstack(channel_stacks)
    flat_stacked_planes = single_stack.flatten()
    # flat_stacked_planes_multiple = channel_stacks[:].flatten()
    print("\n")
    print("Not flat dimension list ch1: ",channel_stacks[0].shape)
    print("Not flat dimension list ch2: ",channel_stacks[1].shape)
    print("Not flat dimension list ch3: ",channel_stacks[2].shape)
    print("Dimension single stack: ",single_stack.shape)
    # print("Flat dimension multiple stacks ",flat_stacked_planes_multiple.shape)
    print("Flat dimension single stack: ",flat_stacked_planes.shape)
    # print("Flat all channels dimension: ",flat_stacked_planes.shape)
    #Calculating percentile
    p_val = np.percentile(flat_stacked_planes, per)
    ...
    datafoldername = biosample+"_"+"patches_"+str(int((datasize/1000)))+"k"
    ...
    for t in range(t_train):    
        ...
        image_AB = get_patch(l,uby,ubx,patchsize,channels,channel_stacks,p_val,t)
