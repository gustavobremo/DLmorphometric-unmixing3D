...
tiles = 49
datasize = 1000
brightnessRange = [0.5,4]
prob = 80
biosample = "bc_org_s_br_webl_nucl_"+str(prob)+"_odN_"+str(per)
datatype = "synthetic"
...
def get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t,brightnesslist,prob,datatype):
        print("Processing patch")   
        #defining alphas for weighted blending
        # alphas = np.arange(0.2, 0.8, 0.01).tolist()
        alphas = np.arange(0.5, 0.9, 0.01).tolist()
        ...
        for c in range(channels):
            ...
            print("percentile: ",percentiles[2])     
            normalized_patch[normalized_patch> 255] = 255
            channel_normalized_patches_list.append(normalized_patch)
        
        #Defining channels from list
        membrane_channel = channel_normalized_patches_list[0].copy()
        nuclear_channel = channel_normalized_patches_list[1].copy()
        open_detector_channel = channel_normalized_patches_list[2].copy()



        #Generating random alpha values for
        alpha1 = round(random.choice(alphas),2)
        alpha2 = round(1-alpha1,2)
        ...
        if datatype == "real":
            ...
            source_image[:,:,0] = open_detector_channel
            source_image[:,:,1] = open_detector_channel
            source_image[:,:,2] = open_detector_channel
            #-------------------------------------------------------------
        elif datatype == "synthetic":
            #-------------------------------------------------------------  
            # SYNTHETIC DATASET    
            # synthetic_patch = (nuclear_channel.astype(np.float64)+membrane_channel.astype(np.float64))/2
            synthetic_patch = cv2.addWeighted(nuclear_channel.astype(np.float64), alpha1,membrane_channel.astype(np.float64),alpha2, 0)
        ...
        target_image[:,:,0] = channel_normalized_patches_list[1]
        target_image[:,:,1] = channel_normalized_patches_list[0]
        ...
        #HISTOGRAM MATCHING
        #----------------------------------------------------------
        # matched_histograms = exposure.match_histograms(real_image,target_image, multichannel=True)
        # gray_img = color.rgb2gray(matched_histograms)

        # synthetic_image = np.zeros((512, 512,3))
        # synthetic_image[:,:,0] = gray_img
        # synthetic_image[:,:,1] = gray_img
        # synthetic_image[:,:,2] = gray_img
        #----------------------------------------------------------
...
# --------------------------------------------------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------------------------------------------------
def main_DataGenerator(filepath,per,patchsize,channels,l_layer,u_layer,tiles,biosample,datasize,brightnessRange,prob,datatype):
    ...
    #----------****Generating & Saving patches****----------
    #Extracting patches for training testing and validation
    print("Training set ************")
    for t in range(t_train):    
        print("Patch: ",t,"generating") 
        image_AB = get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t,brightnesslist,prob,datatype)
        matplotlib.image.imsave(datafoldername+"/train/"+str(t)+'.png', image_AB.astype(np.uint8))
    # print("Testing set ************")
    # for t in range(t_test):  
    #     image_AB = get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t,brightnesslist,prob)
...
main_DataGenerator(filepath,per,patchsize,channels,l_layer,u_layer,tiles,biosample,datasize,brightnessRange,prob,datatype)
