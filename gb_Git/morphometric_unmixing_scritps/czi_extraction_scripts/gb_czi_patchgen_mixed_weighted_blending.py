...
tiles = 49
datasize = 1000
biosample = "bc_organoid_mixed_weighted_belnding_"



#FUNCTION RETURN PATCH AS AB IMAGE
# def get_patch(l,uby,ubx,patchsize,cr,ch_list,ch_percentile,p):
def get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t,alphas,status):
        ...
        if status == "train" or status == "val":
            if t % 2 == 0:
                ...
                #Generate array with synthetic randomly weighted blending for the nuclear and membrane channel
...
def main_DataGenerator(filepath,per,patchsize,channels,l_layer,u_layer,tiles,biosample,datasize):
    ...
    #----------********----------
    #USE THIS LOOP TO CREATE MULTIPLE DATASETS WITH DIFFERENT ALPHA BLENDING
    # for i in range(2,10,2):
    # alpha1 = round((i*.1),1)
    # alpha2 = round((1.0-alpha1),1)

    alphas = np.arange(0.2, 0.8, 0.01).tolist()

    mini = round(min(alphas),2)
    maxi = round(max(alphas),2)
    datafoldername = biosample+str(mini)+"_"+str(maxi)+"_"+"patches_"+str(int((datasize/1000)))+"k"
    ...
    for t in range(t_train):    
        ...
        image_AB = get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t,alphas,"train")
        matplotlib.image.imsave(datafoldername+"/train/"+str(t)+'.png', image_AB.astype(np.uint8))
    print("Testing set ************")
    for t in range(t_test):  
        image_AB = get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t,alphas,"test")
        matplotlib.image.imsave(datafoldername+"/test/"+str(t)+'.png', image_AB.astype(np.uint8))
    print("Validating set ************")
    for t in range(t_val):  
        image_AB = get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t,alphas,"val")
