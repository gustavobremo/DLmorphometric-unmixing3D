...
# filepath = "/hpc/pmc_rios/gbremo/projects/master_thesis/data/VMP_RL1_Exp001_VirtualMultiplexingGT/VMP_RL1_Exp001_Img008_mix5RD.czi"
per = 99
patchsize = 512
channels = 3
l_layer = 101
...
tiles = 49
datasize = 20
biosample = "bc_organoid_quality_check"
...
def get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t):
        ...
        source_image1 = np.zeros((patchsize, patchsize,3))
        source_image2 = np.zeros((patchsize, patchsize,3))
        target_image = np.zeros((patchsize, patchsize,3))
            

        #Real DATA patch CREATED
        real_data_patch = channel_normalized_patches_list[2]
        real_data_patch = real_data_patch.astype(np.uint8)
            
        #Initializing array with normalized channel 3 values (Nuclear and Membrane same fluorophore)
        source_image1[:,:,0] = real_data_patch
        source_image1[:,:,1] = real_data_patch
        source_image1[:,:,2] = real_data_patch



        #SYNTHETIC DATA patch CREATED
        ...
        #Initializing array with normalized synthetic data
        source_image2[:,:,0] = synthetic_patch
        source_image2[:,:,1] = synthetic_patch
        source_image2[:,:,2] = synthetic_patch
        ...
        return(source_image1,source_image2,target_image)
        # return(0)
# --------------------------------------------------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------------------------------------------------
...
def main_DataGenerator(filepath,per,patchsize,channels,l_layer,u_layer,tiles,biosample,datasize):
    ...
    for t in range(t_test):  
        A,B,C = get_patch(l,uby,ubx,patchsize,channels,channel_stacks,percentiles,t)
        matplotlib.image.imsave(datafoldername+"/test/"+str(t)+"r"+'.png', A.astype(np.uint8))
        matplotlib.image.imsave(datafoldername+"/test/"+str(t)+"s"+'.png', B.astype(np.uint8))
        matplotlib.image.imsave(datafoldername+"/test/"+str(t)+"t"+'.png', C.astype(np.uint8))
