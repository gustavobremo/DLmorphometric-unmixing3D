...
**GenerateData.sh** is the bash script where all the parameters can be entered for running **DataGenerator.py**. The following is an example of a bash script used to generate **real** training and testing data consisting of 10 images and normalized using the 99th percentile of each channel and extracted from the layer 67 to 69. More information on parameter options can be found in [command line arguments](https://github.com/gustavobremo/DLmorphometric-unmixing3D/blob/main/Datasets/DataGenerator.py#L278) from **DataGenerator.py**.
